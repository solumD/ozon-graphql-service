...
- Для получения комментариев выбрана cursor-based пагинация. Курсор строится на основе пары `(createdAt, id)`, это дает независимость от offset/limit.
...
### Запуск
Скопировать содержимое файла `.env.example` в файл `.env`, поменять значения при необходимости. Для запуска должен быть установлен Docker Compose.
...
Будут подняты контейнеры с PostgreSQL и приложением, после чего автоматически накатятся миграции с помощью утилиты `goose`.

#### Запуск в in-memory режиме
Приложение запускается без PostgreSQL. Команда сама переопределяет:
...
Будет поднят только контейнер приложения, без БД и без выполнения миграций.
